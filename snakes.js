...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Circle {
        ...
        clone(){
            let clone = new Circle(this.x, this.y, Math.max(this.radius, 1), this.color)
            return clone
        }
        draw() {
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Paddles{
        constructor(){
            this.pad1 = new Rectangle(100, 100, 20, 100, "red")
            this.pad2 = new Rectangle(1180, 100, 20, 100, "blue")
            this.speed = 10
        }
        draw(){
            if(keysPressed['w']){
                this.pad1.y-= this.speed
            }
            if(keysPressed['s']){
                this.pad1.y+= this.speed
            }
            if(keysPressed['i']){
                this.pad2.y-= this.speed
            }
            if(keysPressed['k']){
                this.pad2.y+= this.speed
            }
            this.pad1.draw()
            this.pad2.draw()
            if(this.pad1.doesPerimeterTouch(snake.head)){
                snake.head.xmom*=-1.01
                snake.head.ymom += (Math.random()-.5)*3
                snake.head.move()
            }
            if(this.pad2.doesPerimeterTouch(snake.head)){
                snake.head.xmom*=-1.01
                snake.head.ymom += (Math.random()-.5)*3
                snake.head.move()
            }
    ...
    class Snake{
        constructor(){
            this.head = new Circle(640, 360, 20, "#00FF33", (Math.random()-.5)*15,  (Math.random()-.5)*3.7, 1, 1)
            this.tail = []
            this.length = 90
        }
        draw(){
            ...
            for(let t = 0;t<this.tail.length;t++){
                this.tail[t].draw()
                if(t > 0){
                    this.tail[t].radius-=(20/(this.length *2))
                }
            }
            this.tail.splice(this.length, 50000)
            this.head.draw()
        }
    }
    ...
    class Food{
        constructor(){
            this.body = new Circle(Math.random()*1000,Math.random()*700,7, "red")
        }
        draw(){
            this.body.draw()
            if(snake.head.doesPerimeterTouch(this.body)){
                this.body.x = (Math.random()*1000)+100
                this.body.y = (Math.random()*700)
                snake.length+=15
            }
        }
    }
